...
use num_traits::{CheckedDiv, One, ToPrimitive};
...
/// FastExp type used in binary exponentiation
#[derive(Debug, PartialEq)]
pub struct FastExp<'a> {
    exp: &'a BigUint,
    pub total: BigUint,
    residue: BigUint,
}
...
/// are A and B in the same congruence class mod M
/// # Example
/// ```
...
fn witness(a: BigUint, exp: Factor2n, m: BigUint, count: u64) -> Option<u64> {
    let cong = fast_exponentiation(a, exp.xp(), &m).unwrap();
    ...
    if cong == One::one() {
        // this is a probable prime mod m to base a, lets run it again to be sure with a new base a
        let a_ceil = m.to_u64().unwrap();
        let new_a = BigUint::from(fastrand::u64(1..a_ceil));
        return witness(new_a, exp, m, count + 1);
    }
    cong.to_u64()
}
...
/// # Examples
/// ```
/// assert_eq!(Some(1), miller_rabin(417u64));
/// assert_eq!(None, miller_rabin(42u64));
/// assert_eq!(Some(1), miller_rabin(9973u64));
/// assert_eq!(Some(1), miller_rabin(2u64));
/// assert_eq!(None, miller_rabin(0u64));
/// assert_eq!(None, miller_rabin(u64));
/// assert_eq!(None, miller_rabin(18_446_744_073_709_551_615u64));
/// // don't be fooled by a Carmichael Number!
/// assert_eq!(None, miller_rabin(561));
/// assert_eq!(None, miller_rabin(41041));
/// ```
pub fn miller_rabin(n: u64) -> Option<u64> {
    // exit early when we can
    if n <= 2 && n > 0 {
        return Some(1);
    }
    if n % 2 == 0 {
    ...
    // choose a random base A between 1 -> n
    fastrand::seed(99);
    let a: BigUint = BigUint::from(fastrand::u64(1..n));
    let witness = witness(a, f, big_n, 0u64);
    witness
        .map(|w| match w {
            1 => Some(w),
            _ => None,
...
fn miller_test() {
    ...
    assert_eq!(Some(1), miller_rabin(9973u64));
    assert_eq!(None, miller_rabin(10000u64));
    assert_eq!(Some(1), miller_rabin(1u64));
    assert_eq!(Some(1), miller_rabin(2u64));
    assert_eq!(None, miller_rabin(18_446_744_073_709_551_615u64));
    assert_eq!(None, miller_rabin(561));
    assert_eq!(None, miller_rabin(41041));
...
fn fact2xp_test() {
    let f = Factor2n {
        e: 3,
        k: &BigUint::from(31u32),
    };
    assert_eq!(BigUint::from(248u32), f.xp());
}

// transform a big num into a vector of positions having set bit
...
// transform a list of exponents E into a list of 2^E big nums
...
fn pows_test() {
    ...
    assert_eq!(
        _pows(into_bin(BigUint::from(23u32))),
        vec![
            BigUint::from(1u32),
            BigUint::from(2u32),
            BigUint::from(4u32),
...
/// determine the residue for A^e mod m using binary modular exponentiation
/// # Example
/// ```
/// // 15¹¹⁸¹ mod 41 ≡ 25
/// assert_eq!(
///   fast_exponentiation(
///     BigUint::from(15_u64),
///     BigUint::from(1181_u64),
///     &BigUint::from(41u64)
///  ),
///     Some(BigUint::from(26_u32))
///  );
///  ```
...
fn fastexponent_test() {
    assert_eq!(
        fast_exponentiation(
            BigUint::from(17_u32),
            BigUint::from(23_u32),
            &BigUint::from(121_u32)
        ),
        Some(BigUint::from(51_u32))
    );
    assert_eq!(
        fast_exponentiation(
            BigUint::from(5_u64),
            BigUint::from(117_u64),
            &BigUint::from(19u64)
        ),
        Some(BigUint::from(1_u32))
    );
    assert_eq!(
        fast_exponentiation(
            BigUint::from(15_u64),
    ...
    assert_eq!(
        fast_exponentiation(
            BigUint::from(91_u64),
            BigUint::from(19721_u64),
            &BigUint::from(87u64)
        ),
        Some(BigUint::from(13_u32))
    );
    assert_eq!(
        fast_exponentiation(
            BigUint::from(192_u64),
            BigUint::from(39288849120094815_u64),
            &BigUint::from(71u64)
        ),
        Some(BigUint::from(30_u32))
    );
}
...
/// check whether a list of numbers are pairwise coprime
/// # Example
/// ```
/// assert!(true, pairwise_coprime(vec![&5_u32, &12_u32, &19_u32]));
/// assert!(false, pairwise_coprime(vec![&5_u32, &10_u32, &15_u32]));
/// ```
